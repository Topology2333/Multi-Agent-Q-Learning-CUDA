...
public:
    MultiAgentGridWorld(int size, int n_mines, std::pair<int,int> flag_pos, int n_agents)
        : size_(size), n_mines_(n_mines), flag_pos_(flag_pos), n_agents_(n_agents) {
    ...
    int getActiveCount() const {
        int c = 0;
        for(bool b : active_agents_) if(b) c++;
        return c;
    }

    std::pair<int,int> getFlagPos() const {
        return flag_pos_;
    }

    bool isMineCellPublic(int x, int y) {
        return isMine(x, y);
    }
...
private:
    ...
    void placeMines() {
        ...
        while(placed < n_mines_) {
            ...
            if((rx == 0 && ry == 0) || (rx == flag_pos_.first && ry == flag_pos_.second)) {
                continue;
            }
            if(grid_[rx][ry] != -1) {
                grid_[rx][ry] = -1;
...
public:
    MultiAgentQLearningAgent(MultiAgentGridWorld &env, double alpha=0.1, double gamma=0.9, double epsilon=0.1)
        : env_(env), alpha_(alpha), gamma_(gamma), epsilon_(epsilon) {
        int s = env_.getSize();
        q_table_.resize(s, std::vector<std::vector<double>>(s, std::vector<double>(4, 0.0)));
        std::srand((unsigned)std::time(nullptr));
    }
    ...
    void visualizePolicyGIF(const std::string &filename, int maxSteps=200, int cellSize=20) {
        ...
        int s = env_.getSize();
        int n_agents = env_.getNumAgents();
        auto states = env_.reset();
        std::vector<bool> done_flags(n_agents, false);
        ...
        for(int stepCount = 0; stepCount < maxSteps; stepCount++) {
            ...
            for(int x = 0; x < s; x++) {
                for(int y = 0; y < s; y++) {
                    ...
                    } else if(x == env_.getFlagPos().first && y == env_.getFlagPos().second) {
                        for(int px = 0; px < cellSize; px++) {
                            for(int py = 0; py < cellSize; py++) {
            ...
            for(int ag = 0; ag < n_agents; ag++) {
                if(!done_flags[ag]) {
                    allDone = false;
                    int ax = states[ag].first;
                    int ay = states[ag].second;
                    for(int px = 0; px < cellSize; px++) {
                        for(int py = 0; py < cellSize; py++) {
            ...
            for(int ag = 0; ag < n_agents; ag++) {
                if(!done_flags[ag]) {
                    ...
                    auto stepResult = env_.step(ag, act);
                    states[ag] = std::get<0>(stepResult);
                    int rew = std::get<1>(stepResult);
                    bool dn = std::get<2>(stepResult);
                    updateQ(ax, ay, act, rew, states[ag]);
                    if(dn) done_flags[ag] = true;
                }
            }
            if(activeCount == 0) break;
            if(activeCount < (int)(0.2 * n_agents)) break;
        }
...
protected:
    ...
    void updateQ(int x, int y, int action, int reward, std::pair<int,int> nextState) {
