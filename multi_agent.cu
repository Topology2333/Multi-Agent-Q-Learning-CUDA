...
__device__ void atomicUpdateQ(int x, int y, int action, float tdTarget, float alpha) {
...
void placeMinesHost(int size, int n_mines, int flag_x, int flag_y, std::vector<int>& h_grid) {
    int placed = 0;
...
// For demonstration, we'll only visualize the final Q policy for one agent, ignoring multi-agent aspects in the GIF
// Because the multi-agent environment runs in parallel on GPU
void visualizePolicyGIF(const std::string &gifPath, int size, int flag_x, int flag_y,
                        const std::vector<int> &h_grid, const std::vector<float> &h_Q,
                        int cellSize, int gifMaxSteps) {
...
int main(int argc, char** argv) {
    ...
    for(int i = 1; i < argc; i++) {
        ...
        } else if(std::strcmp(argv[i], "--max_steps_per_episode") == 0 && i+1 < argc) {
    ...
    CHECK_CUDA(cudaMemcpy(d_gridPtr, h_grid.data(), gridSizeInBytes, cudaMemcpyHostToDevice));

    float* d_QPtr;
    CHECK_CUDA(cudaMalloc(&d_QPtr, qSizeInBytes));
    CHECK_CUDA(cudaMemcpy(d_QPtr, h_Q.data(), qSizeInBytes, cudaMemcpyHostToDevice));

    int* d_agentXPtr;
    CHECK_CUDA(cudaMalloc(&d_agentXPtr, agentSizeInBytes));
    CHECK_CUDA(cudaMemcpy(d_agentXPtr, h_agentX.data(), agentSizeInBytes, cudaMemcpyHostToDevice));

    int* d_agentYPtr;
    CHECK_CUDA(cudaMalloc(&d_agentYPtr, agentSizeInBytes));
    CHECK_CUDA(cudaMemcpy(d_agentYPtr, h_agentY.data(), agentSizeInBytes, cudaMemcpyHostToDevice));

    int* d_activePtr;
    CHECK_CUDA(cudaMalloc(&d_activePtr, agentSizeInBytes));
    CHECK_CUDA(cudaMemcpy(d_activePtr, h_active.data(), agentSizeInBytes, cudaMemcpyHostToDevice));
    ...
    CHECK_CUDA(cudaMemcpyToSymbol(d_MAX_STEPS_PER_EPISODE, &max_steps_per_episode, sizeof(int)));
    ...
    __global__ void initRandKernel(SimpleCurand* randStates, unsigned int seed);
    // redeclare or do it inline? We'll do it inline here to avoid confusion, but let's reuse the same signature:
    // We must define it above or we won't compile. It's above. We'll call it now:
    ...
    for(int ep = 0; ep < episodes; ep++) {
        ...
        while(stepCount < max_steps_per_episode) {
            ...
            CHECK_CUDA(cudaMemcpy(d_done, h_done.data(), n_agents*sizeof(int), cudaMemcpyHostToDevice));
            ...
            int h_countActive = 0;
            CHECK_CUDA(cudaMemcpy(&h_countActive, d_countActive, sizeof(int), cudaMemcpyDeviceToHost));
    ...
    CHECK_CUDA(cudaMemcpy(h_Q.data(), d_QPtr, qSizeInBytes, cudaMemcpyDeviceToHost));

    printPolicyCPU(h_grid, h_Q, size, flag_x, flag_y);
    visualizePolicyGIF(gifPath, size, flag_x, flag_y, h_grid, h_Q, cellSize, gifMaxSteps);
